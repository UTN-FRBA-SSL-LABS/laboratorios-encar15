...
> R:la función carrito_init cumple el rol de inicializador de la estructura. Es importante llamarla antes de usar el carrito para establecer los valores de control internos de la estructura Carrito en un estado inicial válido.
...
> R:esta en la linea 24

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Falta multiplicar por la cantidad del producto
...
> R:El bug fue detectado por test_compra_con_descuento que es de integracion, la diferencia es que un test unitario verifica una única función por separado para ver si calcula bien un valor puntual, y uno de integracion verifica que varias funciones colaboren correctamente entre sí
...
> R: En C, los arreglos son bloques contiguos de memoria sin verificación automática de límites en tiempo de ejecución, por lo que al desbordarse escribe directamente en memoria contigua ajena. En cambio en Python o Java la máquina virtual valida cada acceso por índice, y si se desborda interrumpe la ejecución.
...
> R:Es mejor con este enfoque ya que podes entender de manera mas clara en donde y como esta fallando.
...
> R:Cuando cantidad = 1, la multiplicación por 1 no altera el valor del precio unitario. El cálculo incorrecto casualmente arrojó el mismo resultado numérico que el cálculo correcto.
